...
async def pm_text(bot, message):
    ...
    await message.reply_text(
         text=f"<b>ʜᴇʏ {user} 😍 ,\n\nʏᴏᴜ ᴄᴀɴ'ᴛ ɢᴇᴛ ᴍᴏᴠɪᴇs ꜰʀᴏᴍ ʜᴇʀᴇ. ʀᴇǫᴜᴇsᴛ ɪᴛ ɪɴ ᴏᴜʀ <a href=https://t.me/movies_bot_suport>ᴍᴏᴠɪᴇ ɢʀᴏᴜᴘ</a> ᴏʀ ᴄʟɪᴄᴋ ʀᴇǫᴜᴇsᴛ ʜᴇʀᴇ ʙᴜᴛᴛᴏɴ ʙᴇʟᴏᴡ 👇</b>",   
         reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("📝 ʀᴇǫᴜᴇsᴛ ʜᴇʀᴇ ", url=f"https://t.me/movies_bot_suport")]])
    )
...
async def filter_seasons_cb_handler(client: Client, query: CallbackQuery):
    ...
    if settings["button"]:
        ...
        btn.insert(0, 
            [
                InlineKeyboardButton("ꜱᴇɴᴅ ᴀʟʟ ♻️", callback_data=f"sendfiles#{key}"),
                InlineKeyboardButton("ʟᴀɴɢᴜᴀɢᴇs", callback_data=f"languages#{key}"),
                InlineKeyboardButton("Sᴇᴀsᴏɴs",  callback_data=f"seasons#{key}")
            ])
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    # don't change anything without contacting me @LazyDeveloperr
        ...
        buttons = [[
                    ...
                    InlineKeyboardButton('📢 SEARCH', url="https://t.me/filefilterv1bot")
                ],[
        ...
        buttons = [[
            InlineKeyboardButton('⟸ Bᴀᴄᴋ', callback_data='paid'),
        ...
        buttons = [[
            InlineKeyboardButton('⇍ ʙᴀᴄᴋ ⇏', callback_data='paid')
        ]]
        reply_markup = InlineKeyboardMarkup(buttons)
        await query.message.edit_text(
            text="● ◌ ◌"
...
async def auto_filter(client, msg, spoll=False):
    ...
    if settings["button"]:
        ...
        btn.insert(0, [
            InlineKeyboardButton("⛔️ ʜᴏᴡ ᴛᴏ ᴏᴩᴇɴ ᴛʜɪꜱ ʟɪɴᴋ ⛔️", url="https://t.me/")
        ])
    else:
        btn = []
        btn.insert(0, 
